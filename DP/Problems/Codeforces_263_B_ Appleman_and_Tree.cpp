 ...
 * Link: http://codeforces.com/contest/461/problem/B
 */
		...
		int dfs(int node, int parent) {
			...
			rep(i,0,sz(nodes[node].edges)) {
				...
				if(edge.f!=parent) {
					...
					mul[0] = mod(nodes[node].dp[0]*nodes[edge.f].dp[1]);
					mul[0] = mod(mul[0] + mod(nodes[node].dp[0]*nodes[edge.f].dp[0]));

					mul[1] = mod(nodes[node].dp[1]*nodes[edge.f].dp[0]);
					mul[1] = mod(mul[1] + nodes[node].dp[0]*nodes[edge.f].dp[1]);
					mul[1] = mod(mul[1] + nodes[node].dp[1]*nodes[edge.f].dp[1]);

					nodes[node].dp[0] = mul[0];
					nodes[node].dp[1] = mul[1];
				}
			}
		}
}tree;
