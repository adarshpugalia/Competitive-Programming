 ...
 * Problem Link: http://codeforces.com/contest/559/problem/C
 */
...
void solve(int test_case) {
	...
	rep(i,0,sz(v)) {
		// no. of ways to reach cell i, without going through any of the cells before.
		dp[i] = get_nCr(v[i].f+v[i].s-2, v[i].f-1);
		rep(j,0,i-1) {
			if(v[i].f>=v[j].f && v[i].s>=v[j].s) {
				dp[i] = mod(dp[i]-mod(dp[j]*get_nCr(v[i].f-v[j].f+v[i].s-v[j].s, v[i].f-v[j].f)) + MOD);
			}
		}
	}
