...
class Circle {
	public:
		 ...
		 * returns 1, if they don't intersect.
		 * returns 0, if they touch or intersect.
		 * returns -1, if one circle is completely inside the other.
		 */
		int is_intersecting(Circle c) {
			double dis = centre.get_distance(c.centre);
			if(dis>radius+c.radius) return 1;
			if(dis>max(radius, c.radius)-min(radius, c.radius)) return 0;
			return -1;
		}


		/*
		 * This method checks if a point in inside the circel.
		...
		// 	double val = (p.x - centre.x)*(p.x - centre.x) + (p.y - centre.y)*(p.y - centre.y) - radius*radius;
		// 	if(val<0) return 1; else if(val>0) return -1; return 0;
		// }

		/* input and output functions. */
		friend ostream &operator<<(ostream &output, const Circle &c) { output<<c.centre.x<<" "<<c.centre.y<<" "<<c.radius; return output; }
		friend istream &operator>>(istream &input, Circle &c) { input>>c.centre.x>>c.centre.y>>c.radius; return input; }
};
