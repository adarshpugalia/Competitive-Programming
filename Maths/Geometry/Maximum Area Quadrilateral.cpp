 ...
 * Problem: 	Given n points, find the quadrilateral with maximum area with the given points.
 * Complexity: 	O(n^3) time and O(n) space.
 * Link: 		http://codeforces.com/contest/340/problem/B
 */
...
/* This stores the points in the set. */
vector<Point> pts;
int main() {
	...
	rep(i,0,sz(pts)) {
		/* Fixing a diagonal for the quadrilateral. */
		rep(j,i+1,sz(pts)) {

			/* Finding the maximum positive and negative triangle formed with this diagonal. */
			double p_area = 0, n_area = 0;
			rep(k,0,sz(pts)) {
				if(k!=i && k!=j) {
					double area = pts[i].get_area(pts[j], pts[k]);
					if(area>0 && area>p_area)
						p_area = area;
					if(area<0 && abs(area)>n_area)
						n_area = abs(area);
				}
			}

			/* Updating max_area_quadrialteral. */
			if(p_area>0 && n_area>0 && p_area+n_area>max_area)
				max_area = p_area+n_area;
		}
	}
