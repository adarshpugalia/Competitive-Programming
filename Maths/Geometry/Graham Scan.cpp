 ...
 *
 * NOTE: You should make sure all the operations are 2D for convex hull.
 */
class Point {
	public:
		double x, y, z;
...
/*
 * This class implements the convex hull algorithm (2D).
 *
 * @data pts:  the set of points on the plane.
 * @data hull: the set of points that form the convex hull.
 *
 * NOTE: Call the method init() for each new set of points to clear the 
 * vectors pts and hull.
 */
class GrahamScan {
	public:
		vector<Point> pts;
		...
		/* This method clears the pts and hull vectors for reuse. */
		void init() { pts.clear(); hull.clear(); }


		/* This method adds a point to the set pts. */
		void add_point(Point a) { pts.pb(a); }


		/* This is a comparator to sort the points according to polar angle wrt to the lowest point. */
		static bool GrahamScanComparator(Point i, Point j) {
			double turn = lowest_point.calculate_turn(i, j);
			if(db_db_cmp(turn, 0)) {
				if(db_db_cmp(lowest_point.get_distance(i), lowest_point.get_distance(j)))
					return i.z<j.z;

				return lowest_point.get_distance(i)<lowest_point.get_distance(j);
			}
			return turn>0;
		}


		/* This method computes the lowest point (in terms of y cordinate, and then x cordinate) of the set. */
		void compute_lowest_point() {
			lowest_point = pts[0];
			rep(i,1,sz(pts)) if(pts[i].y<lowest_point.y || (pts[i].y==lowest_point.y && pts[i].x<lowest_point.x)) lowest_point = pts[i];
		}


		/* This method computes the convex hull of the set of points. */
		void compute_hull() {
			compute_lowest_point();
			sort(all(pts), GrahamScanComparator);

			hull.pb(pts[0]); hull.pb(pts[1]);
			rep(i,2,sz(pts)) {
				while(hull.size()>1) {
					Point last = hull[sz(hull)];

					/* break if duplicate point. */
					if(last==pts[i]) {
						if(pts[i].z<last.z)
							hull[sz(hull)] = pts[i];
						break;
					...
					hull.pop_back();
					double turn = hull[sz(hull)].calculate_turn(last, pts[i]);

					/*
					 * This exludes collinear points from the hull.
					 * In case, points on the hull are to be included,
					 * change the condition to turn >= 0.
					 */
					if(turn>0) { hull.push_back(last); hull.push_back(pts[i]); break; }
				}

				if(hull.size()==1) hull.pb(pts[i]);
			}
		}


		/* This method returns the perimeter of hull. */
		double get_hull_length() {
			double ret = 0;
			rep(i,1,sz(hull)) ret += hull[i].get_distance(hull[i-1]);
			ret += hull[sz(hull)].get_distance(hull[0]);
			return ret;
		}
}gs;
