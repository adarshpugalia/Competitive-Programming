...
// a brick cannot be above air or water.
bool check_brick_stability(int n, int m) {
	rep(i,0,n-2) {
		rep(j,0,m-1) {
			if(board[i][j]=='B' && (board[i+1][j] == 'W' || board[i+1][j] == 'A'))
				return false;
		}
	}
...
bool check_water_stability(int n, int m) {
	// water can't be present at the side edges.
	rep(i,0,n-1) {
		if(board[i][0] == 'W' || board[i][m-1] == 'W')
			return false;
	}
 
	// water can't be present above or beside air.
	rep(i,0,n-1) {
		rep(j,0,m-1) {
			if(i<n-1 && board[i][j] == 'W' && board[i+1][j] == 'A')
				return false;
 
			if(j>0 && board[i][j] == 'W' && board[i][j-1] == 'A')
				return false;
 
			if(j<m-1 && board[i][j] == 'W' && board[i][j+1] == 'A')
				return false;
		}
	}
...
void solve(int test_case) {
	...
	if(!check_brick_stability(n, m)) {
		cout<<"no\n";
		return;
	}
 
	if(!check_water_stability(n, m)) {
		cout<<"no\n";
		return;
	}
