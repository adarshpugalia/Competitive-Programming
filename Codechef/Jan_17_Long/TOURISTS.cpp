...
class EulerPath {
	public:
		...
		EulerPath() {};
		EulerPath(int n) { g = Graph(n); }
		EulerPath(int n, int m) { g = Graph(n, m); }
...
int main() {
	...
	if(ep.has_eulerian_circuit()) {
		cout<<"YES\n";
		ep.get_eulerian_circuit(1);
		rep(i,0,sz(edges)) {
			if(ep.eulerEdgeSet.find(edges[i]) == ep.eulerEdgeSet.end()) {
